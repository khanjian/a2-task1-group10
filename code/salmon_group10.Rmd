...
oregon_gg <- ggplot(data = oregon, aes(x = long,
  ...
  scale_fill_manual(values = c("turquoise3", "forestgreen")) +
...
clack_gg <- ggplot(data = clackamas, aes(x = long, 
                                       y = lat)) +
  coord_fixed(1.3) +
  geom_polygon(color = "black", fill = "turquoise3") +
  ggmap::theme_nothing() +
  geom_polygon(color = "black", fill = NA) +
  geom_image(data = location_ladder, aes(x = long,
                                         y = lat,
                                         image = image),
             size = 0.1) 


clack_gg / (plot_spacer() + oregon_gg ) +
  plot_annotation(title = "Map of Oregon State and Clackamas County",
...
```{r fig.cap = "**Figure 2**. Mean count of adult Coho, Jack Coho, and Steelhead passage at the Willamette fish ladder between 2001-2010. Data: Columbia River DART. 2021."}
